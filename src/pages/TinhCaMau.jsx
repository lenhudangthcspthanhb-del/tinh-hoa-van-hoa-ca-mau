  ...
  return (
    <div className="min-h-screen" style={{ background: '#f0f4ff' }}>
      ...
      <div className="pt-[72px]">
        ...
        {/* Search Section */}
        <div className="bg-white px-4 py-5 shadow border-y border-gray-200">
          <div className="max-w-6xl mx-auto">
            <p className="text-center text-2xl font-bold text-emerald-600 mb-1">🔍 Tra cứu đơn vị hành chính tỉnh Cà Mau</p>
            <p className="text-center text-sm text-gray-500 mb-4">Tìm theo tên, mã, xã/phường, quận/huyện cũ</p>
            <div className="flex flex-col md:flex-row gap-3">
              <input type="text" placeholder="Nhập tên xã/phường, mã hoặc quận/huyện cũ..." value={query} onChange={e => setQuery(e.target.value)}
                className="flex-1 border border-gray-300 rounded-lg px-4 py-2 text-sm focus:outline-none focus:border-emerald-400" />
              <select value={loaiFilter} onChange={e => setLoaiFilter(e.target.value)}
                className="border border-gray-300 rounded-lg px-4 py-2 text-sm focus:outline-none focus:border-emerald-400 bg-white text-gray-600 min-w-[160px]">
                <option value="all">Chọn loại đơn vị</option>
                <option value="phuong">Phường</option>
                <option value="xa">Xã</option>
              </select>
              <select value={qhFilter} onChange={e => setQhFilter(e.target.value)}
                className="border border-gray-300 rounded-lg px-4 py-2 text-sm focus:outline-none focus:border-emerald-400 bg-white text-gray-600 min-w-[180px]">
                <option value="all">Chọn Quận/Huyện cũ</option>
                {QUAN_HUYEN.map(q => <option key={q} value={q}>{q}</option>)}
              </select>
              <button onClick={clearFilters} className="flex items-center gap-2 bg-red-500 hover:bg-red-600 text-white px-5 py-2 rounded-lg font-bold text-sm transition-colors">
                <span className="material-symbols-outlined text-base">delete</span> Xóa dữ liệu
              </button>
            </div>
          </div>
        </div>
